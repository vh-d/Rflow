...
Rflow is an R package providing R users with a general-purpose workflow framework. It is suitable for various purposes: from simple automation to building powerfull ETL tools.
...
It saves your time as your objects are rebuild only when its needed (also most objects are persistent over sessions).
...
* **environments** serves as containters for nodes. For example a database is a container for tables.
...
* formal job classes (with possibility to keep R/SQL code in standalone R scripts)
