...
py_node <- R6::R6Class(
  ...
  public    = list(
    ...
    print = function(...) {
      super$print()
      cat("  python: ", crayon::cyan(head(as.character(self$py_code))), "\n", sep = "")
    },
    ...
    eval = function(verbose = TRUE, verbose_prefix = "") {
      if (verbose) {
        cat(verbose_prefix, crayon::red(self$id), ": Evaluating R expression:\n", sep = "")
        cat(verbose_prefix, self$py_code)
      }

      # for referencing other objects in rflow
      # .RFLOW <- parent.env(self)

      assign(self$name, reticulate::py_eval(self$py_code), pos = self$r_env)
